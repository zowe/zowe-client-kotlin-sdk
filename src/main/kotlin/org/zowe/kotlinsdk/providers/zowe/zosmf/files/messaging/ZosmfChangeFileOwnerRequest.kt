 ...
 * @property body the request body to execute chown function on a UNIX System Services file or directory
 */
class ZosmfChangeFileOwnerRequest(
  connection: HttpConnection,
  override val filePath: String,
  @property:AvailableSince(ZVersion.ZOS_2_1) override val body: ZosmfChangeFileOwnerRequestBody,
  headers: ZosmfUtilitiesRequestHeaders = ZosmfUtilitiesRequestHeaders(),
) : ZosmfUssUtilitiesRequest(connection, filePath, headers), ChangeFileOwnerRequest {
  override val responseClass = ZosmfChangeFileOwnerResponse::class.java

  override suspend fun produceZosmfResponseObject(
    clientResponse: HttpResponse,
    errorReport: ZosmfErrorReport?
  ): ZosmfHttpResponse {
    return ZosmfChangeFileOwnerResponse(ZosmfStatus(clientResponse.status, errorReport))
  }
}
