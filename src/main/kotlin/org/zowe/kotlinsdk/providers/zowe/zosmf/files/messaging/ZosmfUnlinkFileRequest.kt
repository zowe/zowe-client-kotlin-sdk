 ...
 * @property body the request body to execute unlink function on a UNIX System Services file or directory
 */
class ZosmfUnlinkFileRequest(
  connection: HttpConnection,
  override val filePath: String,
  @property:AvailableSince(ZVersion.ZOS_2_1) override val body: ZosmfUnlinkFileRequestBody,
  headers: ZosmfUtilitiesRequestHeaders = ZosmfUtilitiesRequestHeaders()
) : ZosmfUssUtilitiesRequest(connection, filePath, headers), UnlinkFileRequest {
  override val responseClass = ZosmfUnlinkFileResponse::class.java

  override suspend fun produceZosmfResponseObject(
    clientResponse: HttpResponse,
    errorReport: ZosmfErrorReport?
  ): ZosmfHttpResponse {
    return ZosmfUnlinkFileResponse(ZosmfStatus(clientResponse.status, errorReport))
  }
}
