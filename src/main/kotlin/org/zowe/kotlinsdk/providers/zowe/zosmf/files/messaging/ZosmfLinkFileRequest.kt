 ...
 * @property body the request body to execute link function on a UNIX System Services file or directory
 */
class ZosmfLinkFileRequest(
  connection: HttpConnection,
  override val filePath: String,
  @property:AvailableSince(ZVersion.ZOS_2_1) override val body: ZosmfLinkFileRequestBody,
  headers: ZosmfUtilitiesRequestHeaders = ZosmfUtilitiesRequestHeaders()
) : ZosmfUssUtilitiesRequest(connection, filePath, headers), LinkFileRequest {
  override val responseClass = ZosmfLinkFileResponse::class.java

  override suspend fun produceZosmfResponseObject(
    clientResponse: HttpResponse,
    errorReport: ZosmfErrorReport?
  ): ZosmfHttpResponse {
    return ZosmfLinkFileResponse(ZosmfStatus(clientResponse.status, errorReport))
  }
}
