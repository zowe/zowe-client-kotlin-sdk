 ...
 * @property body the request body to execute chtag function on a UNIX System Services file or directory
*/
class ZosmfChangeFileTagRequest(
  connection: HttpConnection,
  override val filePath: String,
  @property:AvailableSince(ZVersion.ZOS_2_1) override val body: ZosmfChangeFileTagRequestBody,
  headers: ZosmfUtilitiesRequestHeaders = ZosmfUtilitiesRequestHeaders(),
) : ZosmfUssUtilitiesRequest(connection, filePath, headers), ChangeFileTagRequest {
  override val responseClass = ZosmfChangeFileTagResponse::class.java

  override suspend fun produceZosmfResponseObject(
    clientResponse: HttpResponse,
    errorReport: ZosmfErrorReport?
  ): ZosmfHttpResponse {
    return ZosmfChangeFileTagResponse(
