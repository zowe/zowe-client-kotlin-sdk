 ...
 * @property body the request body to execute cp function on a UNIX System Services file or directory
 */
class ZosmfCopyFileRequest(
  connection: HttpConnection,
  override val filePath: String,
  @property:AvailableSince(ZVersion.ZOS_2_1) override val body: ZosmfCopyFileRequestBody,
  headers: ZosmfUtilitiesRequestHeaders = ZosmfUtilitiesRequestHeaders(),
) : ZosmfUssUtilitiesRequest(connection, filePath, headers), CopyFileRequest {
  override val responseClass = ZosmfCopyFileResponse::class.java

  override suspend fun produceZosmfResponseObject(
    clientResponse: HttpResponse,
    errorReport: ZosmfErrorReport?
  ): ZosmfHttpResponse {
    return ZosmfCopyFileResponse(ZosmfStatus(clientResponse.status, errorReport))
  }
}
