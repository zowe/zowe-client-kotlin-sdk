 ...
 */
class ZosmfJesAPIImpl(val connection: Connection, val httpClient: OkHttpClient) : JesAPI {
  // TODO: doc
  override fun getJob(params: GetJobRequest): GetJobResponse {
    ...
    return if (hasNameAndId)
      GetJobByNameAndIdOperation(zosmfParams, connection, httpClient).runOperation()
    else if (hasCorrelator)
      GetJobByCorrelatorOperation(zosmfParams, connection, httpClient).runOperation()
    else
